...
audioFilePath = "MusicBoxNotesV2_fixed.wav" 
...
wav.write("MusicBoxNotesMono.wav", fs, monoRecording)
# Print out some data about the audio file
t = np.linspace(0, len(monoRecording)/fs, len(monoRecording))
sampleNumber = np.linspace(0, len(monoRecording) - 1, len(monoRecording))

# Define the figure for plotting
audioPlot = plt.figure(figsize=(12, 6))
# Plot the original audio signal
plt.plot(sampleNumber, monoRecording)
plt.xlabel("Time (s)")
